...
class OnnxScorerAdaptater(ICreditScorer):
    ...
    def charger(self) -> None:
        ...
        # -- 1. Session ONNX -------------------------------------------------
        journalapp.info("Chargement ONNX : %s", chemin_onnx)
        ...
        journalapp.info("ONNX pret | provider : %s", providers[0])

        # -- 2. Preprocesseur sklearn ----------------------------------------
        journalapp.info("Chargement preprocesseur : %s", chemin_preproc)
        ...
        journalapp.info(
            "Preprocesseur charge | %d features apres encodage",
            # len(self._noms_features_enc),
            self._nb_features,
        )
        
        # -- 3. Modele LightGBM original pour SHAP ---------------------------
        journalapp.info("Chargement LightGBM (SHAP) : %s", chemin_lgbm)
        self._modele_lgbm = joblib.load(chemin_lgbm)

        # -- 4. Background SHAP (sous-ensemble du dataset de reference) ------
        if chemin_ref.exists():
            journalapp.info("Chargement background SHAP depuis : %s", chemin_ref)
            ...
            journalapp.info(
                "Background SHAP : %d lignes", nb_bg
            )
        else:
        ...
        # -- 5. TreeExplainer SHAP (compilation une fois) --------------------
        #journalapp.info("Initialisation TreeExplainer SHAP...")
        ...
        #    model_output     = "probability",  # Outputs en probabilite [0,1]
        #)
        #journalapp.info("TreeExplainer SHAP pret.")

        # -- 5. TreeExplainer SHAP (en onnx_scorer_adapter.py) --------------------
        # -- 5. TreeExplainer SHAP -------------------------------------------
        journalapp.info("Initialisation TreeExplainer SHAP...")
        
        try:
            # Intento 1: Modo estándar con background data
            self._explainer_shap = shap.TreeExplainer(
                model           = self._modele_lgbm,
                data            = self._background_shap,
                feature_names   = self._noms_features_enc,
        ...
        except (shap.utils._exceptions.ExplainerError, ValueError) as e:
            journalapp.warning(f"Adaptation TreeExplainer requise : {e}")
            
            # Intento 2: Modo robusto compatible con variables categóricas nativas
            # Obligatorio: data=None y model_output="raw"
            self._explainer_shap = shap.TreeExplainer(
                model                = self._modele_lgbm,
                data                 = None,
                feature_perturbation = "tree_path_dependent",
                model_output         = "raw",  # Único modo soportado aquí
            )

        journalapp.info("TreeExplainer SHAP prêt (mode tree_path_dependent).")
        ...
        if chemin_meta.exists():
            ...
            journalapp.info(
                "Metadonnees : run_id=%s | auc=%.4f | seuil=%.3f",
                meta.get("run_id", "?"),
        ...
        if hasattr(self._preprocesseur, "feature_names_in_"):
            ...
            journalapp.info("Colonnes attendues par le préprocesseur : %s", self._nb_features)  #  self._columnas_originales
        else:
    ...
    def predire(self, demande: "DemandeCredit") -> "DecisionCredit":
        ...
        X_float32 = X_transforme.astype(np.float32)  # shape (1, N)
        ...
        # -- Etape 4 : Calcul SHAP values ------------------------------------
        # TreeExplainer opere sur X_float64 (pas float32 -- conversion auto)
        shap_values = self._calculer_shap(X_transforme)  # ndarray shape (N,)

        # -- Etape 5 : Agregation SHAP espace original -----------------------
        explications = self._agreger_shap(shap_values, demande)

        # -- Etape 6 : Construire la decision --------------------------------
        score = ScoreRisque(valeur=probabilite_defaut)
        
        decision = score.vers_decision(self._seuil)

        journalapp.debug(
            "Prediction | proba=%.4f | decision=%s | latence=%.1f ms | "
            "top_shap=%s (+%.3f)",
            probabilite_defaut,
            decision.value,
            latence_ms,
            explications[0].nom_feature if explications else "?",
            explications[0].impact_shap if explications else 0.0,
        )
